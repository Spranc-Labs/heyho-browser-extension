...
async function setupWakeAlarm() {
  try {
    ...
    console.log(
      `✅ Wake alarm created: fires every ${HEARTBEAT_CONFIG.alarmIntervalMinutes} minute(s)`
    );
  } catch (error) {
...
function ensureHeartbeatRunning() {
  ...
  if (timeSinceLastHeartbeat > expectedInterval * 2) {
    console.log(
      `⚠️ Heartbeat interval appears dormant (${Math.round(timeSinceLastHeartbeat / 1000)}s since last beat)`
    );
